...
dot = Digraph(comment='The NFA Table')
dot_DFA = Digraph(comment='The DFA Table')
...
# 这边是栈的操作  '-'就是NFA中的空置连接符
def make_NFA(stri,transword):
    stack = []  #Link栈
    for i in stri:
        if i in transword:#如果是转换式如a\b\c等,入栈
            ...
            newLink = Link('_')
            catchLink = stack.pop()
            newLink.turn_around_self(catchLink)  # 迭代
            stack.append(newLink)
...
def print_catch_link(node,queue):
    ...
    queue.append(node)            # 每个加入队列中，并且value编号
...
def NFA_relation(queue):  #打印NFA
    for node in queue:
        ...
        pir=str(node.value)    # 不能直接这样转换

        dot.node(name=pir,label=pir)
        for i in range(len(node.nextlist)):
            print(node.tranlist[i], '->', node.nextlist[i].value)
            # 在创建两圆点之间创建一条边
            if  node.tranlist[i] is not '_':
                dot.edge(pir, str(node.nextlist[i].value), node.tranlist[i])
            else:
                dot.edge(pir, str(node.nextlist[i].value),'ε')
...
def make_DFA_small(statelist,queue,alltrans,words):#DFA最小化
    ...
    for i in range(len(new_state_tran_list)):
        ...
        if len(new_state_list[i]) is not 0:   # 把空的排除掉
            dot_DFA.node(name=str(i), label=str(new_state_list[i]))
        # dot_DFA.node(name=str(i), label=str(new_state_list[i]))
        ...
        for k in new_state_tran_list[i].keys():
            if len(new_state_list[new_state_tran_list[i][k]]) is not 0:  # 指向的地方不是空的
                dot_DFA.edge(str(i), str(new_state_tran_list[i][k]), k)
            # dot_DFA.edge(str(i), str(new_state_tran_list[i][k]), k)
...
if __name__ == '__main__':
    ...
    #  (a+b)*c
    # ==========获得正则表达式==========#
    get_str = input("输入对应的正规式:   ")
    print("你输入的正规式为:   ",get_str)
    # ==========获取可能的输入串==========#
    wordlist = getWord(get_str)
    print('你的转化的输入串列表为:', wordlist)
    # ==========将正则式变为后缀式========#
    get_str = reserve.reserve(get_str)
    print('正规式转化成为的后缀式为：',get_str)
    # ==========建造NFA===================#
    NFA = make_NFA(get_str,wordlist)           # 返回的是整个link
    # ==========打印NFA===================#
    queue = []  # NFA状态队列
    print_catch_link(NFA.getfirstNode(),queue)     # 编号并且顺序加入队列中queue
    NFA_relation(queue)          # 打印a
    set_isend_inNFA(queue)
    # =========得到DFA初始状态============#
    print('DFA初始状态为：')
    state = set()  # 存放初始状态
    getbegin(NFA.getfirstNode(),state)        # 打印初始状态
    print(state)
    # 通过初始状态生成DFA（没有化简的版本）#
    statelist = [state]  # 初始的生成DFA只有一个状态
    DFA = getDFA(queue,statelist,wordlist)  # 获得DFA  返回的是一个矩阵，abc的下标代表着经过该元素生成的子集
                                            # set()代表经过该元素无法生成子集
                                            # statelist为储存状态子集的列表
                                            # queue保存每个结点
    ...
    dot.render('show_NFA/NFA.gv', view=True)

    print(dot_DFA.source)
    # 保存source到文件，并提供Graphviz引擎
    dot_DFA.render('show_DFA/NFA.gv', view=True)
